...

{
	...
	function getversion(){
		$version = $this->cut_str($this->curl("https://github.com/hogeunk/vng270_hoge", "", ""), 'Revision ','<');
...
class stream_get extends getinfo
{
	...
	function fulllist()
	{
		...
		if ($act != "") {
			if ((isset($_POST['checkbox'][0]) && $_POST['checkbox'][0] != null) || isset($_POST['renn']) || isset($_POST['remove'])) {
				echo '<table style="width: 500px; border-collapse: collapse" border="0" align="center"><tr><td><center>';
				switch ($_POST['option']) {
				case 'del':
						echo "<div style='position:absolute;background-color:rgba(226,60,60,0.53);left:0;top:0;right:0;bottom:0;z-index:14' ></div>
                         <div style='position:absolute;top:15%;left:135px;right:140px;z-index:15' >
                          <div style='box-shadow:0 0 5px red;width:43%;font-size:20px;padding:9px;margin-bottom:5px;font-family:arial;background-color:#fff;border-radius:20px' dir='rtl'>
						    <lable>آیا از حذف فایل انتخاب شده اطمینان دارید ؟</lable>";
						$this-> deljob();
						echo "
                         </div  >
                          </div >";
		...
		if (count($files) == 0) {
			echo "<center><div style='box-shadow:0 0 5px red;width:43%;font-size:20px;padding:9px;margin-bottom:5px;font-family:arial;background-color:#fff;border-radius:20px' dir='rtl'>" . $this->lang['notfile'] . "</div></center><br>";
			return;
		}
		...
		echo "<div style='padding-top:6px;'></div><center><a class='chmenu' href=javascript:setCheckboxes(1)> {$this->lang['checkall']} </a>  <a class='chmenu' href=javascript:setCheckboxes(0)> {$this->lang['uncheckall']} </a> </center><br/>";
		...
		echo '</select>';
		echo '<div style="overflow: auto; height: auto; max-height: 450px; width: 800px;"><table id="table_filelist" class="filelist" align="left" cellpadding="3" cellspacing="1" style="width: 800px;max-width: 800px;min-width: 800px;" ><thead><tr class="flisttblhdr" valign="bottom" style="background-color: #4b433b;"><td style="width:200px;max-width:200px;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;" class="sorttable_alpha"><b>' . $this->lang['name'] . '</b></td>'.($this->directdl ? '<td ><b>'.$this->lang['direct'].'</b></td>' : '').'<td style="width:200px;max-width:200px;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;"><b>' . $this->lang['original'] . '</b></td><td style="width:100px;max-width:100px;"><b>' . $this->lang['size'] . '</b></td><td style="width: 5px;max-width:5px;" id="file_list_checkbox_title" class="sorttable_checkbox">&nbsp;</td></tr></thead><tbody>';
		usort($files, array(
			$this,
			'datecmp'
		));
      ...
      <tr class='flistmouseoff' align='center'>
		...
		echo "</tbody><tbody><tr class='flisttblftr'><td>&nbsp;</td><td></td><td></td><td><div style='direction: rtl;'><b>" . $this->lang['total'] . " :</b> $totalall</div></td><td></td></tr></tbody></table>
				</div></form><br/>";

		else echo "</tbody><tbody><tr class='flisttblftr'><td>&nbsp;</td><td></td><td><div style='direction: rtl;'><b>" . $this->lang['total'] . " :</b> $totalall</div></td><td></td></tr></tbody></table>
				</div></form><br/>";
	}
	...
	{ 
		...
		if (isset($_POST['checkbox'])) {
			...
			echo "<input style='width: 33px; height: 23px' type='submit' value='" . $this->lang['dell'] . "' name='remove'/> &nbsp; <input style='width: 33px; height: 23px' type='submit' value='" . $this->lang['canl'] . "' name='Cancel'/>";
		}
...
*/			}
		...
		if($dl){
		echo "<center><div style='box-shadow:0 0 5px red;width:43%;font-size:20px;padding:9px;margin-bottom:5px;font-family:arial;background-color:#fff;border-radius:20px' dir='rtl'>$dl &#1601;&#1575;&#1610;&#1604; &#1581;&#1584;&#1601; &#1711;&#1585;&#1583;&#1610;&#1583; .</div></center>";
		}
...

{
	...
	function convert($filesize)
	{
		...
		if (preg_match('/^([0-9]{1,4}+(\.[0-9]{1,2})?)/', $filesize, $value)) {
			if (srtipos($filesize, "TB")||srtipos($filesize, "TB")) $value = $value[1] * 1024 * 1024 * 1024 * 1024;
			elseif (srtipos($filesize, "GB")||srtipos($filesize, "GB")) $value = $value[1] * 1024 * 1024 * 1024;
			elseif (srtipos($filesize, "MB")||srtipos($filesize, "MB")) $value = $value[1] * 1024 * 1024;
			elseif (srtipos($filesize, "KB")||srtipos($filesize, "KB")) $value = $value[1] * 1024;
			else $value = $value[1];
		}
		else $value = 0;
		return $value;
	...
	function convertmb($filesize)
	{
		...
		if ($filesize >= 1024 * 1024 * 1024 * 1024) $value = round($filesize / (1024 * 1024 * 1024 * 1024) , 2) . " TB";
		elseif ($filesize >= 1024 * 1024 * 1024) $value = round($filesize / (1024 * 1024 * 1024) , 2). " GB";
		elseif ($filesize >= 1024 * 1024) $value = round($filesize / (1024 * 1024) , 2) . " MB";
		elseif ($filesize >= 1024) $value = round($filesize / (1024) , 2) . " KB";
